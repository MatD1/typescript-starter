...
**Cache**: 15 s under a mode- and service-reference-specific key.
...
| `scheduledTripId` | `String`        | No       | Planner/scheduled ID used when it differs from realtime |
| `routeId`         | `String`        | No       | Exact GTFS route fallback identity                      |
| `startDate`       | `String`        | No       | GTFS service date (`yyyyMMdd`)                          |
| `startTime`       | `String`        | No       | GTFS service start time (`HH:mm:ss`)                    |
...
`itdDate` and `itdTime` must be supplied together. The server removes journeys
outside the requested boundary before caching and returns `searchMode`,
`requestedDateTime`, `generatedAt`, and `timezone` metadata with the trips.
...
`ServiceReferenceObject` contains realtime and scheduled trip IDs, exact route
identity when available, server-derived mode, service date, and start time. Pass
these fields to `trackTrip` so matching does not depend on one fragile ID.
